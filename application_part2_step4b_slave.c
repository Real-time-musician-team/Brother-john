...
CONTROLLER controller = { initObject(),120,0,450,950,200,0,1,1};
...
void receiver(App *self, int unused) {
    ...
    else if (msg.msgId==1&&msg.nodeId==2)
    {
        SCI_WRITE(&sci0, "Can msg received: \'");
        SCI_WRITE(&sci0,msg.buff);
        SCI_WRITE(&sci0, "\'\n");
    ...
    else if (msg.msgId==3&&msg.nodeId==2)
    {
    ...
    if (msg.buff[0]>= 251 && msg.buff[0]<= 255) 
                    {
                      ...
                      else if (msg.buff[0]>= 0 && msg.buff[0]<= 5)
                      {
                        SCI_WRITE(&sci0, "The entered key is: \'");
                        SCI_WRITE(&sci0, output);
                        SCI_WRITE(&sci0, "\'\n");
    ...
    else if (msg.msgId==4&&msg.nodeId==2)
    {
        ...
        if(msg.buff[0]>= 60 && msg.buff[0]<= 240)
                    {
                        ...
                        SCI_WRITE(&sci0, output);
                        SCI_WRITE(&sci0, "\'\n");
                    ASYNC(&controller,tempoin, msg.buff[0]);
                    }
        else{
    ...
    else if (msg.msgId==5&&msg.nodeId==2)
    {
    ...
    SCI_WRITE(&sci0, "\'\n");
                        ...
                        SCI_WRITE(&sci0, output);
                        SCI_WRITE(&sci0, "\'\n");
                    //ASYNC(&controller,step, msg.buff[0]);
...
{
    AFTER(MSEC(c*(self->a_period)),&controller,generate,0); 
}*/
